...
def save_publications_to_json(faculty_tuple: tuple[str, str]):
    print(faculty_tuple[0])
    faculty_pubs = []
    if faculty_tuple[1]:
        author = scholarly.fill(scholarly.search_author_id(faculty_tuple[1]))
        for publication in author['publications']:
            faculty_pubs.append(publication['bib'])
    with open(faculty_tuple[0] + '.json', 'w', encoding='utf-8') as f:
...
for faculty in faculty_in_department[:n]:
